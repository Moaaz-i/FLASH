...
async function runBenchmark() {
  console.log('\n===============================================================');
  console.log('⚡ FLASH (FlashDB) High-Performance Engine Benchmark ⚡');
  console.log('===============================================================\n');
  ...
  // FlashBinary Zero-Copy Single Field Lookup
  const flashBuf = FlashBinary.serialize(sampleDoc);
  const t0_fb = performance.now();
  for (let i = 0; i < 10000; i++) {
    const val = FlashBinary.getField(flashBuf, 'email');
  }
  ...
  // 2. Cryptographic Throughput: AES-256-GCM & Blind Indexing
  ...
  for (let i = 0; i < 5000; i++) {
    const enc = cipher.encrypt('Sensitive Data Payload 2026');
    const trapdoor = blind.generateTrapdoor('email', 'test@domain.com');
  }
  ...
  // 3. Database End-to-End Write & Read Operations in Temp OS Folder
  ...
  try {
    ...
    console.log(`   - Encrypted Writes (WAL + SkipList + Indexes): ${writeOps.toLocaleString()} ops/sec (${(writeDuration / 2000).toFixed(2)} ms/op)`);

    // Encrypted Reads
    const t0_read = performance.now();
    for (let i = 0; i < 1000; i++) {
      const idx = Math.floor(Math.random() * 2000);
      const res = await col.findOne({ email: `user${idx}@benchmark.io` });
    }
    ...
    merkleRoot = col.raw.getMerkleRoot();
