...
/**
 * FLASH key-agreement helpers (FlashPQC).
 * ECDH on secp256k1 + SHA3-256. Not NIST ML-KEM / Kyber.
 */
export class FlashPQC {
  ...
  static generateKeyPair() {
    ...
    const secretKeyBuf = Buffer.concat([
      priv,
      crypto.createHash("sha256").update(priv).digest(),
    ]);

    const pubCompressed = ecdh.getPublicKey(null, "compressed"); // 33 bytes
    const pubPadding = crypto
      .createHash("sha256")
      .update(pubCompressed)
      .digest()
      .subarray(0, 31); // 31 bytes
  ...
  static encapsulateSecret(peerPublicKeyHex) {
    ...
    const ctPadding = crypto
      .createHash("sha256")
      .update(alicePubCompressed)
      .digest()
      .subarray(0, 31);
    const ciphertext = Buffer.concat([alicePubCompressed, ctPadding]).toString(
      "hex",
    );
   ...
   */
  static deriveQuantumHardenedKey(
    passphrase,
    salt = "flash_pqc_lattice_salt_2026",
  ) {
    // Stage 1: Strong Key Derivation using scryptSync (much more secure than PBKDF2)
    const intermediate = crypto.scryptSync(passphrase, salt, 32, {
      N: 16384,
      r: 8,
      p: 1,
    });
