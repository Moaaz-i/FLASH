...
import {
  assertLocalOrExplicitPublicBind,
  clientIp,
  createIpRateLimiter,
  setSecurityHeaders,
  timingSafeCompare,
...
export class FlashServer {
   ...
   * @param {FlashRBAC} [options.rbac] - Optional operation RBAC (header x-flash-user)
   * @returns {http.Server}
   */
  static start(options = {}) {
    ...
    const rbac = options.rbac instanceof FlashRBAC ? options.rbac : null;
    assertLocalOrExplicitPublicBind(host, {
      allowPublicBind: options.allowPublicBind === true,
      authKey,
    });
    const allowRequest = createIpRateLimiter();

    const db = new FlashDatabase(dbName, { storagePath, engineOptions });
    const metrics = new FlashMetrics();

    const authorize = (req, collection, action) => {
      if (!rbac) return;
      const userId = req.headers["x-flash-user"];
      if (!userId || !rbac.can(String(userId), collection, action)) {
    ...
    const zkFail = (err) =>
      ...
      err.message.startsWith("Zero-knowledge violation");
    ...
    const server = http.createServer(async (req, res) => {
      ...
      res.setHeader(
        ...
        "Content-Type, x-flash-server-key, x-flash-user",
      );
      ...
      // Server Authentication Verification (always required)
      const clientKey = req.headers["x-flash-server-key"];
      if (!clientKey || !timingSafeCompare(clientKey, authKey)) {
        res.writeHead(401, { "Content-Type": "application/json" });
        return res.end(
          JSON.stringify({ error: "Unauthorized: Invalid server authKey" }),
        );
      }
      ...
      if (url.pathname === "/metrics" && req.method === "GET") {
        ...
        for (const colName of db.listCollections()) {
          try {
            ...
            await col.init();
            metrics.setGauge(
              `storage_${colName}_sstables`,
              col.sstables.length,
            );
            metrics.setGauge(
              `storage_${colName}_memtable_bytes`,
              col.memtable.byteSize,
            );
            metrics.setGauge(
              `storage_${colName}_doc_count`,
              col.docOrder.length,
            );
          } catch {}
        }
      ...
      const readBody = (limitBytes = 10 * 1024 * 1024) =>
      ...
      if (url.pathname.startsWith("/api/v1/query/") && req.method === "POST") {
        const colName = decodeURIComponent(
          url.pathname.replace("/api/v1/query/", ""),
        );
        try {
          const { envelope, options } = await readBody();
          authorize(req, colName, "read");
          ...
          logger.info("FlashServer", "find completed", {
            collection: colName,
            durationMs,
          });
          res.writeHead(200, { "Content-Type": "application/json" });
          return res.end(
            JSON.stringify({
              records: wireRecords,
              merkleRoot:
                typeof col.getMerkleRoot === "function"
                  ? col.getMerkleRoot()
                  : "",
            }),
          );
          ...
          logger.error("FlashServer", "find failed", {
            collection: colName,
            error: err.message,
          });
          const status =
            err.statusCode ||
            (zkFail(err) ||
            err.message.includes("Invalid or dangerous collection name")
              ? 400
              : 500);
          ...
          return res.end(
            JSON.stringify({
              error: clientVisible ? err.message : "Internal Server Error",
            }),
          );
        }
      }
      ...
      if (url.pathname.startsWith("/api/v1/insert/") && req.method === "POST") {
        ...
        try {
          ...
          logger.error("FlashServer", "insert failed", {
            collection: colName,
            error: err.message,
          });
          const status =
      ...
      if (
        url.pathname.startsWith("/api/v1/insertMany/") &&
        req.method === "POST"
      ) {
        const colName = decodeURIComponent(
          url.pathname.replace("/api/v1/insertMany/", ""),
        );
        try {
          const { encryptedRecords } = await readBody();
          authorize(req, colName, "write");
          const recordBufs = (encryptedRecords || []).map((r) => {
            const buf = FlashRecordCodec.decodeFromWire(r);
            FlashZKKernel.assertSealedRecord(buf, "FlashServer.insertMany");
            return buf;
          });
          const col = db.collection(colName);
          await col.init();
          ...
          logger.error("FlashServer", "insertMany failed", {
            collection: colName,
            error: err.message,
          });
          const status =
      ...
      if (url.pathname.startsWith("/api/v1/delete/") && req.method === "POST") {
        const colName = decodeURIComponent(
          url.pathname.replace("/api/v1/delete/", ""),
        );
        try {
          const { filter } = await readBody();
          authorize(req, colName, "delete");
          FlashZKKernel.assertBlindQueryEnvelope(
            filter || {},
            "FlashServer.delete",
          );
          ...
          logger.info("FlashServer", "delete completed", {
            collection: colName,
            durationMs,
          });
          res.writeHead(200, { "Content-Type": "application/json" });
          ...
          logger.error("FlashServer", "delete failed", {
            collection: colName,
            error: err.message,
          });
          const status =
      ...
      if (url.pathname.startsWith("/api/v1/flush/") && req.method === "POST") {
        const colName = decodeURIComponent(
          url.pathname.replace("/api/v1/flush/", ""),
        );
        try {
          authorize(req, colName, "admin");
          const col = db.collection(colName);
          await col.flush();
          const durationMs = Date.now() - startOpTime;
          metrics.recordOp("flush", durationMs);
          logger.info("FlashServer", "flush completed", {
            collection: colName,
            durationMs,
          });
          res.writeHead(200, { "Content-Type": "application/json" });
          ...
          logger.error("FlashServer", "flush failed", {
            collection: colName,
            error: err.message,
          });
          const status =
